...
export default function Programmers() {
  ...
  const getNameOfFeatured = () => {
    ...
    const featuredArray = programmers.filter(programmer => programmer.id == celebrate)

    return featuredArray[0].name
  };

  const style = {
    fontSize: '1.5em',
    marginTop: '0.5em',
    color: (celebrate == '' ) ? 'royalblue' : 'gold', // 🤔 color turns to gold, when celebrating
  };
      ...
      <div className='programmers'>
        {
          programmers.map(dev =>
            <div className='programmer' key={dev.id}>
              {dev.name} <button onClick={() => { featureMe(dev.id) }}>Feature</button>
            </div>
          )
        }
      </div>
